...
def cal_PSNR(T_img, E_img):
    '''
    :param T_img:  tensor(B, C, D, H, W)
    :param E_img:  tensor(B, C, D, H, W)
    :return: PSNR (float32)
    '''
    assert len(T_img.shape) == len(T_img.shape), 'T_img/E_img mismatch - {}, {}.'.format(len(T_img.shape),
                                                                                         len(T_img.shape))
    T_img = T_img.clone().squeeze().float().cpu().numpy()
    E_img = E_img.clone().squeeze().float().cpu().numpy()

    d_range = 255.0

    T_img = T_img.astype(np.float64)
    E_img = E_img.astype(np.float64)
    err = np.mean(np.power(T_img - E_img, 2))
    if err == 0:
        return float('inf')
    # return 20 * math.log10(1.0 / math.sqrt(err))
    return 10 * np.log10((d_range ** 2) / err)
...
class SpectralTVLoss(nn.Module):
    def __init__(self, weight_s=0.1, weight_h=1, weight_w=1):
        super(SpectralTVLoss, self).__init__()
    ...
    def forward(self, x):
        ...
        s_tv = torch.abs((x[:, 1:, :, :] - x[:, :-1, :, :])).mean()
        h_tv = torch.abs((x[:, :, 1:, :] - x[:, :, :-1, :])).mean()
        w_tv = torch.abs((x[:, :, :, 1:] - x[:, :, :, :-1])).mean()

        return self.weight_s * s_tv + self.weight_h * h_tv + self.weight_w * w_tv

class SpectralTVLossS(nn.Module):
    def __init__(self):
        super(SpectralTVLossS, self).__init__()

    def forward(self, x):
        s_tv = torch.abs((x[:, 1:, :, :] - x[:, :-1, :, :])).mean()
        return s_tv

class P_Norm_Loss(nn.Module):
    def __init__(self, p=1.2):
        super(P_Norm_Loss, self).__init__()
        self.p = p

    def forward(self, x):

        return (torch.pow(torch.abs(x), self.p).mean())
