...
def win_to_ubuntu_path(win_path: str) -> str:
    # 将盘符替换为 Ubuntu 格式的 /mnt/<盘符小写>
    ubuntu_path = win_path.replace('\\', '/')
    if ':' in ubuntu_path:
        drive_letter = ubuntu_path[0].lower()
        ubuntu_path = f"/mnt/{drive_letter}{ubuntu_path[2:]}"
    return ubuntu_path
def take(x):
